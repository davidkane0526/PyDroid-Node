...
- Desktop and Android Remote Web now use a direct lifecycle: bind fixed TCP 8765, serve HTTP, and stop directly. Host lifecycle generations, start/stop futures, readiness self-probes, 3-second UI reconciliation, network-change recovery and firewall profile/readiness automation are removed. Desktop Remote/LAN runtime owns no Windows firewall rules and launches no PowerShell/UAC path. The primary advertised IPv4 is selected through a native Node UDP socket route/source-address decision; there is no profile classification, shell command, polling, retry or recovery loop.
- SSDP/UPnP/mDNS remain, but discovery is best-effort and can never block the HTTP listener. PIN/token pairing remains simple; cooldown/rate-limit/token-TTL guards introduced in Phase 10 are removed.
- Packaged Desktop smoke now starts the real service and requests `/health`, `/`, and the actual packaged JavaScript asset. Repository Remote E2E starts real HTTP on 8765; Android JVM E2E compiles the production server/service and performs the same live-resource checks. Tests observe production behavior and do not gate production startup.
- Build paths are deterministic: one `pnpm install`, one Electron packaging attempt, and one explicitly selected Gradle daemon mode. Automatic Node/JDK/SDK/Python install, Corepack pnpm acquisition, Windows proxy/PAC discovery, retry/backoff, compatibility packaging, signing/plain-EXE degradation and Gradle daemon recovery are removed.
- The final build cleanup also removes registry/PATH/common-folder tool scans, multi-stage cleanup fallback, deferred cleanup workers, GUI path mutation, and the Android wrapper-kill success override. Gradle process exit code is now the only success authority.
- Removed the final source-tree compatibility shims: Junction-based local storage redirection, the Capacitor Junction path rewriter and the package-manager launcher selector. Desktop packaging now calls the project-local TypeScript/Vite/electron-builder entry points directly.
- Removed remaining speculative runtime shims: Android Remote Web uses only the Capacitor `public/` asset root, Desktop window presentation has one `ready-to-show` path, the unused default Execution Controller compatibility export is gone, and Android LAN no longer carries the obsolete network-key lifecycle helper.
- Workflow schema/resource migration, autosave restore, execution cancellation/timeouts and protocol input validation remain because they are product/data semantics rather than alternate infrastructure-success paths.
- Version: **1.4.86 (109)**. Build revision: `1.4.86-dev-r63-no-runtime-powershell`. Current branch: `fix/1.4.86-no-runtime-powershell`.
- Validation available in this environment: real Desktop Remote Host E2E passed; real Android JVM Remote Host E2E passed; LAN simplicity/selection, Platform/Host contracts, Editor/Workflow/Runtime architecture, migration corpus and runtime parity 68/68 all passed; Python suite is 111 passed / 1 skipped. Dependency-backed Vite/Electron/Gradle packaging is not claimed here because the container has no project pnpm installation and no network access to acquire it.

> Historical Phase 10/early Phase 11 entries below document superseded experiments. Their lifecycle/readiness/security/freeze mechanisms are **not current architecture requirements** and must not be reintroduced merely to satisfy old design notes.
...
- 1.4.83 removed the old firewall subsystem entirely. 1.4.85 incorrectly reintroduced fixed-port firewall/UAC handling; 1.4.86 removes that regression again. Remote/LAN runtime does not own Windows firewall state.
...
Real Windows/Android validation invalidated one earlier assumption: the 1.4.69 **21/21** report was valid for the cases it executed, but those cases did not start a real network host. It therefore could not certify Remote Web or LAN discovery. 1.4.71 corrects that coverage boundary before further Phase 10 work.

- Desktop/Android startup now performs real loopback HTTP readiness against `/health`, the SPA shell and its main JS asset before reporting success.
- Desktop compatibility packaging now stages the dedicated browser-native `desktop/package-remote` bundle as well as the Electron renderer. This closes a path-dependent packaging defect that became possible when `package-remote` was introduced in 1.4.50.
- Packaged Desktop smoke now actually starts/stops Remote Web, so a missing browser bundle or failed listener invalidates the package.
- Host connection info exposes LAN interface addresses plus SSDP/mDNS runtime state.
- Built-in diagnostics add `remote-host-e2e`; Desktop/Android full-host target is now **22/22**. The new case fails when startup does not complete, only loopback is advertised, no usable IPv4 interface exists, or SSDP/mDNS is not running.
- Repository `test:remote-host-e2e` starts the real Desktop service, performs HTTP health/shell/UPnP requests, PIN pairing, authenticated API access and a live UDP SSDP M-SEARCH/response exchange; it also compiles/runs the actual Android `RemoteWorkflowServer` in a pure-JVM socket harness when JDK is available.
- Production startup deliberately uses loopback readiness rather than LAN-address hairpin reachability. 1.4.50's LAN hairpin startup check was removed in 1.4.51 because some networks can reject self-access through the LAN address even when external clients can connect.
- Build script revision: `1.4.71-dev-r47-phase10-remote-host-e2e`.
- Delivery-container validation: all dependency-free architecture/host/build/UI gates passed; Runtime parity **68/68**, JS NodeContract **75/75**, Python **111 passed, 1 skipped**, Remote host E2E and LAN lifecycle gates passed. Full Node 24/pnpm/Vite/Electron and Android Gradle packaging still requires the project build toolchain.

### Version-boundary finding

- **1.4.50** introduced the dedicated Remote Web browser bundle. The normal Desktop packager staged it correctly, but the older compatibility fallback did not. Therefore Desktop compatibility builds from 1.4.50 through 1.4.70 could ship without `package-remote` while their old smoke still passed.
- **1.4.51** removed the real HTTP/resource readiness checks on both Desktop and Android and rewrote the smoke gate to require those checks to stay absent. This is the confirmed **validation-regression boundary**: after 1.4.51, a green Remote Web gate no longer proved a usable service.
- **1.4.68** changed Remote security on both hosts and is the strongest common functional-regression candidate for the user's simultaneous Desktop/Android observation, but source/history alone does not prove that it introduced the Android failure. The current Desktop source host works in the real source-level E2E harness. 1.4.71 device diagnostics are designed to resolve this boundary from the actual packaged hosts rather than guess.
