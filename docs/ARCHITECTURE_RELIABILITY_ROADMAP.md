...
稳定 `main` 基线：`1.4.27 (50)`；Phase 8 已验收基线：`1.4.59 (82)`；当前 Phase 9：`1.4.62 (85)`
...
10. Phase 9 Editor Core & Workspace Session：1.4.60 开始；1.4.62 已继续迁移普通节点事务、连续编辑 history 合并、布局与 save/open/close/autosave restore 生命周期。目标是让每标签页编辑状态、命令与手势解释拥有稳定边界；Desktop/Mobile 与 Node/Group 手势不得被强制统一。
...
状态：**1.4.62 持续开发中，尚未冻结。**

Phase 9 的重点不是重新设计 UI，而是让 `EditorWorkspaceSession` 成为每标签页 graph/input/history/view state 的唯一编辑器状态源，并逐步将用户操作收敛为 Editor Commands。React 负责订阅和呈现，不再作为工作流编辑语义的拥有者。1.4.62 已进一步迁移节点新增/复制/参数修改/布局事务，并让 `EditorWorkspaceLifecycleService` 接管保存、打开、关闭 dirty 判断和显式 autosave restore。正常启动仍保持单个空白工作流，不自动恢复旧画布。
