...
11. **1.4.88 keeps the user-validated 1.4.73 direct-bind + single-flight Remote baseline while restoring only deterministic Windows default-route address selection.** Remote Web uses one production path: direct bind on `0.0.0.0:8765`, immediate startup completion, direct stop, simple PIN/token pairing, and best-effort SSDP/UPnP/mDNS that never gates HTTP. Windows may execute hidden `route.exe PRINT 0.0.0.0` once as a read-only query to mark the default-route address among synchronous `os.networkInterfaces()` results; this query never gates the listener. Desktop Remote/LAN runtime must not launch PowerShell, request UAC elevation, inspect/modify Windows firewall or network profile state, perform active UDP route probes, or introduce readiness/recovery/lifecycle control. Default Desktop build output uses a stable executable path so OS application permissions are not invalidated by version-directory churn. Tests observe the real final artifact and must never replace its startup path.
