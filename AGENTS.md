...
12. **Phase 11 Workflow Compatibility & Migration is implemented in 1.4.78 and awaiting the final one-shot user-host verification. 1.4.77 was rejected by the real Windows build because its new diagnostics code referenced a nonexistent EditorWorkspaceSession API; keep the diagnostics module inside the Phase 11 strict TypeScript gate.** Preserve Workflow schema v3 migration ownership in Workflow Core, resource schema v2 opaque future-version protection, complete Git-history corpus coverage and fail-closed NodeSpec/function migration rules. Do not ask the user for intermediate Phase 11 tests.
