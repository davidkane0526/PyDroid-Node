...
11. **1.4.89 keeps Remote Web on the direct-bind + single-flight baseline and makes the current Windows Desktop path unconditionally stable.** Remote Web uses one production path: direct bind on `0.0.0.0:8765`, immediate startup completion, direct stop, simple PIN/token pairing, and best-effort SSDP/UPnP/mDNS that never gates HTTP. LAN address enumeration is synchronous `os.networkInterfaces()` only. Desktop Remote/LAN runtime must not launch PowerShell, `route.exe` or other external processes, request UAC elevation, inspect/modify Windows firewall or network profile state, perform active route probes, or introduce readiness/recovery/lifecycle control. `KeepHistory` may create a separate version archive but must never change the current runnable `PyDroid-Flow-Desktop` path. Passive runtime logging may record listener/URL/request facts but must never affect service control.
