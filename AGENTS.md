...
11. **1.4.87 restores the user-validated 1.4.73 direct-bind + single-flight Remote baseline without restoring Phase 10 control layers.** Remote Web uses one production path: direct bind on `0.0.0.0:8765`, immediate startup completion, direct stop, simple PIN/token pairing, and best-effort SSDP/UPnP/mDNS that never gates HTTP. LAN addresses come only from synchronous `os.networkInterfaces()` enumeration. Desktop Remote/LAN runtime must not launch PowerShell or any shell command, request UAC elevation, inspect/modify Windows firewall or network profile state, perform an external route probe, or introduce readiness/recovery/lifecycle control. Tests observe the real service and must never replace or delay its startup path.
