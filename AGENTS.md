...
11. **1.4.91 keeps Remote Web direct and makes build cleanup long-path safe without retries.** Remote Web uses one production path: direct bind on `0.0.0.0:8765`, immediate startup completion, direct stop, simple PIN/token pairing, and best-effort SSDP/UPnP/mDNS that never gates HTTP. Desktop Remote/LAN runtime must not launch PowerShell, `route.exe` or other external network processes, request UAC elevation, inspect/modify Windows firewall or network profile state, perform active route probes, or introduce readiness/recovery/lifecycle control. The current runnable path is always `PyDroid-Flow-Desktop`; repeat builds replace it with one `robocopy /MIR` operation rather than recursive pre-delete. Workspace cleanup uses one .NET recursive delete with Windows extended-length paths; do not restore PowerShell `Remove-Item -Recurse`, retries or alternate cleaners. Packaged runtime logging is always `<exe-dir>/logs/desktop.log` and remains observational only.
