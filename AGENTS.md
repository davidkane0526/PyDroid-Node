...
9. **Phase 8 Workflow Language / State & Function System is user-validated and frozen at 1.4.59.** Preserve workspace-variable isolation, schema-v2 reusable functions and dual-runtime parity.
10. **Phase 9 Editor Core & Workspace Session is active from 1.4.60.** `EditorWorkspaceSession` owns per-tab graph/input/history/view state; editing operations should move behind Editor Commands. Gesture semantics must remain policy-driven by both input profile (`desktop`/`mobile`) and target (`node`/`group`/`canvas`/`resource`/`tab`). Do not force Desktop and Android, or nodes and groups, into one gesture definition merely to reduce code.
