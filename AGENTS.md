...
12. **Phase 11 Workflow Compatibility & Migration is implemented in 1.4.81 and awaiting one final real-host verification. 1.4.80 was rejected only by a packaged-smoke Host Contract bridge-name mismatch; all frozen Remote/LAN production files were unchanged. 1.4.77 was rejected by the real Windows TypeScript build; 1.4.78 exposed a same-host LAN false positive; 1.4.79 was rejected because the packaged-build smoke again started/gated the real LAN production path.** Preserve Workflow schema v3 migration ownership in Workflow Core, resource schema v2 opaque future-version protection, complete Git-history corpus coverage and fail-closed NodeSpec/function migration rules. Remote Web/LAN production code is frozen by `test:remote-production-freeze`: fixed TCP 8765 and existing UI/SSDP/mDNS/UPnP/PIN/Token semantics must not change without an explicit defect-backed unfreeze. Diagnostics are observational only; they must never control service startup. Do not add UI copy or ask for intermediate Phase 11 tests.
