...
11. **1.4.86 keeps the 1.4.83 deterministic Remote/LAN contract and closes the 1.4.85 regression.** Remote Web uses one production path: direct bind on `0.0.0.0:8765`, direct stop, simple PIN/token pairing, and best-effort SSDP/UPnP/mDNS that never gates HTTP. Desktop Remote/LAN runtime must not launch PowerShell or any shell command, request UAC elevation, inspect/modify Windows firewall or network profile state, or introduce readiness/recovery/lifecycle control. Primary LAN address selection may use the native Node socket routing decision only. Tests observe the real service and must never replace its startup path.
