...
> **当前开发版本：1.4.80 (103) · Phase 11 Workflow Compatibility & Migration 最终修复候选**。1.4.79 被真实 Windows 构建拒绝：打包 smoke 在构建期启动真实 LAN 服务，并让同步防火墙检查/二次 readiness 参与生产启动，造成 `/health` 自检反向阻断打包。1.4.80 恢复“生产路径最小化、验证旁路化”：固定 8765/SSDP/mDNS/UPnP/PIN/Token 与现有 UI 文案保持不变；Windows LocalSubnet 防火墙集成仅在服务已经成功启动后异步 best-effort 执行，永不决定 start 成败；打包 smoke 不再打开真实 LAN listener。Remote Host 诊断只有观察到真实外部客户端后才可宣称外部可达。详见 [docs/phase11-workflow-compatibility-migration.md](docs/phase11-workflow-compatibility-migration.md)。
