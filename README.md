...
桌面端与移动端现已提供“节点插件”管理入口，可直接选择 JSON Manifest 或 `.plugin.zip`，执行安装、启用、停用和卸载，并查看插件版本、节点类型与 Python/JavaScript Runtime 支持。`.plugin.zip` 固定采用根目录 `manifest.json + js/ + python/ + resources/`，Provider 与资源文件在安装时解析到同一可持久化 Manifest，重启恢复不依赖原 ZIP 路径。JavaScript Provider 使用 `execute(params, upstream, context, api)` 入口，Runtime API v2 暴露原生 `Table` 与只读 `api.resources`；Python Provider 随工作流请求进入桌面、Android 或 Remote backend，并通过 `context["resources"]` 读取同一批只读包资源。节点也可以将声明的资源作为图标。NodeSpec SDK v6 进一步支持宿主统一渲染的声明式 Inspector UI：参数分组、纯文本/包内资源帮助、条件显示、联动枚举、动态 number `min/max/step`、显式 read-only/disabled 编辑状态，以及从最新节点结果读取 kind/value/text/rows/columns 的只读状态摘要；插件不能注入 React、DOM 或任意 HTML，也不能通过这些声明自动改写参数值。当前仍不包含依赖解析、自动更新、签名或 Marketplace。完整格式见 `docs/node-plugin-packages.md`，可直接安装的样例见 Demo 27～36、`examples/plugins/` 与 `examples/plugin-archives/`.
