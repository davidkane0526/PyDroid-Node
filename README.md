...
在 Android 或 Windows 桌面端顶部点击局域网 AirDrop 图标并启动网页访问后，现有 HTTP/Web 服务会自动同时启动 SSDP/UPnP 与 mDNS/DNS-SD 发现。应用显示简洁的 `http://宿主设备IP:端口/`，同时发布稳定的 `.local` 主机名。Windows 电脑可在“文件资源管理器 → 网络”中发现 `PyDroid Node - 设备名`，双击后由 UPnP `presentationURL` 使用默认浏览器打开同一 Web UI，不需要手工输入 IP。
...
桌面端与移动端现已提供“节点插件”管理入口，可直接选择 JSON Manifest 或 `.plugin.zip`，执行安装、启用、停用和卸载，并查看插件版本、节点类型与 Python/JavaScript Runtime 支持。`.plugin.zip` 固定采用根目录 `manifest.json + js/ + python/ + resources/`，Provider 与资源文件在安装时解析到同一可持久化 Manifest，重启恢复不依赖原 ZIP 路径。JavaScript Provider 使用 `execute(params, upstream, context, api)` 入口，Runtime API v2 暴露原生 `Table` 与只读 `api.resources`；Python Provider 随工作流请求进入桌面、Android 或 Remote backend，并通过 `context["resources"]` 读取同一批只读包资源。节点也可以将声明的资源作为图标。NodeSpec SDK v7 进一步支持宿主统一渲染的声明式 Inspector UI：参数分组、纯文本/包内资源帮助、条件显示、联动枚举、动态 number `min/max/step`、显式 read-only/disabled 编辑状态、参数校验提示，以及从最新节点结果或指定输出端口读取 kind/value/text/rows/columns 的只读状态摘要；插件不能注入 React、DOM 或任意 HTML，也不能通过这些声明自动改写参数值。当前仍不包含依赖解析、自动更新、签名或 Marketplace。完整格式见 `docs/node-plugin-packages.md`，可直接安装的样例见 Demo 27～38、`examples/plugins/` 与 `examples/plugin-archives/`.
...
Android 打包需要 JDK 21、完整 64-bit Python 3.13（含 `venv`/`ensurepip`）和 Android SDK platform 36。默认自动发现本机已有安装；例如当前历史成功构建曾使用 `D:\Code\Java`、`C:\Users\dk\AppData\Local\Android\Sdk`、`D:\PyDroidTemp\tools\pydroid-flow\Python\3.13\python.exe`。这些只是已验证实例，不是硬编码要求。Desktop 便携 Python 默认位于 `D:\PyDroidTemp\tools\pydroid-flow\Python\runtime-3.13`。debug APK 输出到 `android/app/build/outputs/apk/debug/app-debug.apk`。
