...
- Revalidates every promoted function operation in TypeScript before materialization: both Python and JavaScript runtimes must be supported, side effects and state are forbidden, and Notebook/Python-function/function-call carrier nodes cannot be smuggled into a portable function graph. Invalid or stale portable metadata therefore falls back to `custom.python_function` instead of creating a falsely JavaScript-capable function.
- Adds native multi-node Workflow Function parity coverage with `pandas.dropna → table.reset_index`; the analyzer-to-workflow-to-runtime path was also exercised end to end with a real `pd.read_csv → clean(...)` Notebook flow on both runtimes.
- Preserves pandas CSV header semantics during Notebook import: `pd.read_csv()` now lowers with explicit `header='infer'` instead of inheriting the palette node's raw-data `header='none'` default. `header=None`, `0` and `1` are retained explicitly.
- Tightens pandas file-reader lowering around the JavaScript parity rule. Only the `read_csv` options implemented by both runtimes are promoted; unsupported/Python-only parser options remain source-authoritative Python. `read_table` is limited to the exactly representable separator/header subset, and `pd.read_json` remains Python because generic record-JSON loading cannot reproduce pandas' object-orientation semantics without inspecting the file contents.
- Local validation: Python **155 passed / 1 skipped**; Runtime Parity **82/82**; JS-capable NodeContract coverage **84/84**; Workflow Compatibility real migration execution and strict TypeScript compatibility pass; Workflow Core, NodeContract, Runtime Engine, UI, baseline, host, Remote Web, LAN and execution architecture smoke gates pass.
- Build revision: `1.5.2-dev-r80-portable-notebook-functions`.
