...
class LanDiscoveryService {
  ...
  start({ port }) {
    ...
    for (const item of this.interfaces) this.log(`[LAN] Interface ${item.name} / ${item.address}${item.preferred ? " (preferred)" : ""}`);
  ...
  primaryAddress() { return this.interfaces.find((item) => item.preferred)?.address ?? this.interfaces[0]?.address ?? "127.0.0.1"; }
  ...
  getStatus() {
    return {
      interfaces: this.interfaces.map((item) => ({ name: item.name, address: item.address, preferred: Boolean(item.preferred) })),
