...
export const NODE_CATALOG: NodeSpec[] = [
  ...
  },
  {
    ...
    inputPorts: [
      ...
      { id: "min", label: "Min", valueType: "number", defaultParameter: "min" },
      { id: "max", label: "Max", valueType: "number", defaultParameter: "max" },
      { id: "seed", label: "Seed", valueType: "number", defaultParameter: "seed" },
    ],
    outputPorts: TABLE_OUTPUT,
    parameters: [
    ...
    variants: [
      ...
      { when: { distribution: "normal" }, inputPorts: [
        ...
        { id: "std", label: "Std", valueType: "number", defaultParameter: "std" },
        { id: "seed", label: "Seed", valueType: "number", defaultParameter: "seed" },
      ], hiddenParameters: ["min", "max"] },
    ],
  },
  {
  ...
  },
  {
    ...
    ui: { inlineParameters: ["value"], inlineParameterLabels: { value: null }, inlineLayout: "stack" },
  },
  {
  ...
  {
    nodeType: "table.column_transform", runtimeSupport: ["python", "javascript"],
    label: "列变换",
    description: "声明一次列变换，不直接修改表格；可由列变换 Pipeline 按顺序批量执行。",
    tags: ["column", "transform", "pipeline", "列", "变换", "批处理"],
    category: "表格处理",
    defaults: { columns: "0", operation: "multiply", operand: 1, operand2: 1 },
    inputPorts: [
      { id: "columns", label: "Columns", valueType: "any", defaultParameter: "columns", socketGroup: "transformData" },
      { id: "operand", label: "Value", valueType: "number", defaultParameter: "operand", socketGroup: "transformValue" },
    ],
    outputPorts: [{ id: "output", label: "Transform", valueType: "object" }],
    socketGroups: [
      { id: "transform", label: "Transform" },
      { id: "transformData", label: "目标列", parentId: "transform" },
      { id: "transformValue", label: "参数", parentId: "transform" },
    ],
    parameters: [
      { key: "columns", label: "目标列", kind: "text", required: true, placeholder: "0,1 或 voltage,current" },
      { key: "operation", label: "运算", kind: "select", options: [
        ...
        { label: "平方", value: "square" }, { label: "log10", value: "log10" }, { label: "ln", value: "ln" },
        { label: "exp", value: "exp" }, { label: "倒数", value: "reciprocal" },
        { label: "Min-Max 归一化", value: "normalize" }, { label: "Z-score 标准化", value: "zscore" },
      ] },
      { key: "operand", label: "标量", kind: "number" },
      { key: "operand2", label: "第二标量", kind: "number", advanced: true },
    ],
    ...
    variants: [
      ...
      { when: { operation: "clip" }, inputPorts: [
        { id: "columns", label: "Columns", valueType: "any", defaultParameter: "columns", socketGroup: "transformData" },
        { id: "operand", label: "Min", valueType: "number", defaultParameter: "operand", socketGroup: "transformValue" },
        { id: "operand2", label: "Max", valueType: "number", defaultParameter: "operand2", socketGroup: "transformValue" },
      ], parameterPatches: { operand: { label: "下限" }, operand2: { label: "上限", advanced: false } } },
    ],
  },
  {
  ...
  },
  {
    ...
    tags: ["column", "pipeline", "transform", "batch", "列", "流水线", "批处理", "多输入"],
    category: "表格处理",
    defaults: { transformCount: 3 },
    socketGroups: [
      { id: "pipeline", label: "Pipeline" },
      { id: "pipelineData", label: "数据", parentId: "pipeline" },
      { id: "pipelineTransforms", label: "Transforms", parentId: "pipeline" },
    ],
    inputPorts: [{ id: "input", label: "表格", valueType: "table", required: true, socketGroup: "pipelineData" }],
    outputPorts: TABLE_OUTPUT,
    parameters: [{ key: "transformCount", label: "变换数量", kind: "number", min: 1, max: 16, step: 1 }],
    ui: { inlineParameters: ["transformCount"], inlineParameterLabels: { transformCount: null }, inlineLayout: "row" },
    inputPortGroups: [{ id: "transforms", socketGroup: "pipelineTransforms", repeat: { countParameter: "transformCount", idPrefix: "transform", labelPrefix: "Transform", valueType: "object", required: true, min: 1, max: 16 } }],
  },
  {
  ...
  },
  {
    ...
    inputPorts: [
      { id: "input", label: "表格", valueType: "table", required: true },
      { id: "groupSize", label: "Period", valueType: "number", defaultParameter: "groupSize" },
      { id: "tailRows", label: "Tail", valueType: "number", defaultParameter: "tailRows" },
    ],
    outputPorts: TABLE_OUTPUT,
    parameters: [
      { key: "groupSize", label: "周期行数", kind: "number", min: 1, step: 1 },
    ...
    ui: { inlineParameters: ["partialGroup"], inlineParameterLabels: { partialGroup: null }, inlineLayout: "row" },
  },
  {
    ...
    category: "表格处理",
    defaults: { chunks: 2 },
    inputPorts: [{ id: "input", label: "表格", valueType: "table", required: true }, { id: "chunks", label: "Chunks", valueType: "number", defaultParameter: "chunks" }],
    outputPorts: TABLE_OUTPUT,
  ...
  },
  {
    ...
    ui: { inlineParameters: ["column"], inlineParameterLabels: { column: "列" }, inlineLayout: "row" },
  },
  {
  ...
  },
  {
    ...
    inputPorts: [{ id: "input", label: "表格", valueType: "table", required: true }, { id: "value", label: "Value", valueType: "any", defaultParameter: "value" }],
    outputPorts: TABLE_OUTPUT,
    parameters: [
  ...
  },
  {
    ...
    category: "Pandas 常用",
    defaults: { n: 5 },
    inputPorts: [{ id: "input", label: "表格", valueType: "table", required: true }, { id: "n", label: "N", valueType: "number", defaultParameter: "n" }],
    outputPorts: TABLE_OUTPUT,
  ...
  },
  {
    ...
    category: "Pandas 常用",
    defaults: { decimals: 2 },
    inputPorts: [{ id: "input", label: "表格", valueType: "table", required: true }, { id: "decimals", label: "Digits", valueType: "number", defaultParameter: "decimals" }],
    outputPorts: TABLE_OUTPUT,
  ...
  },
  {
    ...
    label: "生成固定电平周期波形",
    description: "直接生成高/低固定电平周期波形。repeatCount=1 且从高电平开始时输出 high → low → high；totalTime>0 时按周期持续生成到指定总时长。",
    ...
    parameters: [
      ...
      { key: "lowVoltage", label: "低电平（V）", kind: "number", required: true, step: 0.1 },
      { key: "highTime", label: "高电平持续时间（s）", kind: "number", required: true, min: 0.000001, step: 0.001 },
      { key: "lowTime", label: "低电平持续时间（s）", kind: "number", required: true, min: 0.000001, step: 0.001 },
      { key: "repeatCount", label: "重复次数", kind: "number", required: true, min: 1, max: 49999, step: 1 },
      { key: "startLevel", label: "起始电平", kind: "select", required: true, options: [{ label: "高电平", value: "high" }, { label: "低电平", value: "low" }] },
  ...
  },
  {
    ...
    label: "数值序列表",
    description: "按 Python range 语义生成 iteration/value 两列表格。它是数据生成节点，不是带 BODY 的控制流结构。",
    tags: ["range", "序列", "整数", "生成", "参数扫描"],
    category: "列表处理",
    defaults: { start: 0, stop: 10, step: 1 },
    inputPorts: [
      { id: "start", label: "Start", valueType: "number", defaultParameter: "start" },
      { id: "stop", label: "Stop", valueType: "number", defaultParameter: "stop" },
  ...
  },
  {
    ...
    label: "数值递推",
    description: "按条件和更新表达式生成 iteration/value 递推轨迹。它是独立数据节点，不是带 BODY 的 While Zone。",
    tags: ["递推", "迭代", "数值", "序列", "trajectory"],
    category: "列表处理",
  ...
  },
  {
    ...
    category: "绘图",
    defaults: { y: "1", label: "", group: "", legendGroup: "", visible: true, solo: false, lineStyle: "-", marker: "", lineWidth: 1.5 },
    socketGroups: [
      { id: "data", label: "数据" },
    ...
    inputPorts: [
      ...
      { id: "legendGroup", label: "Legend Group", valueType: "text", defaultParameter: "legendGroup", socketGroup: "identity" },
      { id: "visible", label: "Visible", valueType: "boolean", defaultParameter: "visible", socketGroup: "presentation" },
      { id: "solo", label: "Solo", valueType: "boolean", defaultParameter: "solo", socketGroup: "presentation" },
    ...
    parameters: [
      ...
      { key: "group", label: "数据分组", kind: "text", placeholder: "例如 signal 或 control" },
      { key: "legendGroup", label: "图例分组", kind: "text", placeholder: "例如 drain 或 gate" },
      { key: "visible", label: "显示曲线", kind: "boolean" },
      { key: "solo", label: "单显曲线", kind: "boolean" },
  ...
  },
  {
    ...
    inputPorts: [{ id: "input", label: "整数列表", valueType: "list", required: true }, { id: "minLength", label: "Min Length", valueType: "number", defaultParameter: "minLength" }],
  ...
  },
  {
    ...
    inputPorts: [{ id: "input", label: "对象", valueType: "any", required: true }, { id: "indent", label: "Indent", valueType: "number", defaultParameter: "indent" }], outputPorts: [{ id: "output", label: "JSON 文本", valueType: "text" }], parameters: [{ key: "indent", label: "缩进", kind: "number", min: 0, max: 8, step: 1 }],
  },
  {
  ...
  },
  {
    ...
    category: "Python 内置",
    defaults: { digits: 0 },
    inputPorts: [{ id: "input", label: "数字", valueType: "number", required: true }, { id: "digits", label: "Digits", valueType: "number", defaultParameter: "digits" }],
