...
export function DataGrid({ preview, onExpand }: { preview: TablePreview; onExpand?: () => void }) {
  ...
  return <div className={`data-grid ${wrap ? "data-grid--wrap" : ""} ${compact ? "data-grid--compact" : ""}`} onDoubleClick={onExpand}>
    <div className="data-grid__toolbar"><label className="data-grid__search"><span>⌕</span><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(0); }} placeholder="搜索所有已载入单元格" aria-label="筛选表格"/>{query && <button aria-label="清除筛选" onClick={() => setQuery("")}>×</button>}</label><select className="data-grid__page-size" value={pageSize} onChange={(event) => { setPageSize(Number(event.target.value)); setPage(0); }} aria-label="每页行数"><option value={10}>10 / 页</option><option value={25}>25 / 页</option><option value={50}>50 / 页</option><option value={100}>100 / 页</option></select><div className={`data-grid__actions ${onExpand ? "data-grid__actions--4" : "data-grid__actions--3"}`}><button className={wrap ? "active" : ""} title="单元格自动换行" onClick={() => setWrap((value) => !value)}>↩</button><button className={compact ? "active" : ""} title="紧凑行高" onClick={() => setCompact((value) => !value)}>≡</button><button className={copied ? "copied" : ""} title="复制 TSV，可直接粘贴到 Excel" onClick={() => void copyTable()}>{copied ? "已复制" : "复制"}</button>{onExpand && <button className="data-grid__expand" onClick={onExpand}>全屏</button>}</div></div>
