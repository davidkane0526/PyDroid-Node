...
export function NodePluginManager({ open, onClose }: NodePluginManagerProps) {
  ...
  if (!open) return null;
  return <div className="node-plugin-manager-backdrop" role="dialog" aria-modal="true" aria-label="节点插件">
    <section className="node-plugin-manager">
      <header><strong>节点插件</strong><button type="button" aria-label="关闭节点插件" onClick={onClose}>×</button></header>
