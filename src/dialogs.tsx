...
export function SettingsDialog({ open, themeMode, language, resolvedTheme, runtimePreference, canvas, smbServer, smbShare, smbGuest, smbUsername, smbDisabled, debugMode, hotReloadEnabled, profilePath, workspaceUri, onClose, onThemeModeChange, onLanguageChange, onRuntimePreferenceChange, onCanvasChange, onOpenSmb, onOpenAgent, onDebugModeChange, onConfigureFolder, onExportSettings, onImportSettings }: {
  ...
  return <div className="settings-backdrop" role="dialog" aria-modal="true" aria-label={L("设置", "Settings")}>
    <section className="settings-dialog settings-dialog--adaptive">
      <header><div><strong>{L("设置", "Settings")}</strong><span>{L("界面、运行时与平台配置", "Interface, runtime and platform")}</span><small className="settings-version-badge">v{APP_VERSION}</small></div><button className="dialog-close-button" aria-label={L("关闭设置", "Close settings")} onClick={onClose}><svg viewBox="0 0 20 20" aria-hidden="true"><path d="m5 5 10 10M15 5 5 15"/></svg></button></header>
