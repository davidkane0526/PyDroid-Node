  ...
  return <div className="settings-backdrop" role="dialog" aria-modal="true" aria-label={L("AI Agent 设置", "AI Agent settings")}>
    <section className="settings-dialog agent-dialog">
      ...
      <div className="settings-dialog__body">
        ...
        <section><h3>{L("审计", "Audit")}</h3>{audit.length ? <ol className="agent-audit">{audit.slice(0, 5).map((entry) => <li key={`${entry.at}-${entry.summary}`}><strong>{entry.summary}</strong><span>{entry.result}</span></li>)}</ol> : <p className="muted">{L("尚无 AI 操作记录。", "No AI audit entries yet.")}</p>}</section>
      </div>
    </section>
  </div>;
}
...
export function SettingsDialog({ open, themeMode, language, resolvedTheme, runtimePreference, canvas, smbServer, smbShare, smbGuest, smbUsername, smbDisabled, debugMode, hotReloadEnabled, profilePath, workspaceUri, onClose, onThemeModeChange, onLanguageChange, onRuntimePreferenceChange, onCanvasChange, onOpenSmb, onOpenAgent, onDebugModeChange, onConfigureFolder, onExportSettings, onImportSettings }: {
  ...
  return <div className="settings-backdrop" role="dialog" aria-modal="true" aria-label={L("设置", "Settings")}>
    <section className="settings-dialog settings-dialog--adaptive">
      <header><div><strong>{L("设置", "Settings")}</strong><span>{L("界面、运行时与平台配置", "Interface, runtime and platform")}</span></div><small className="settings-version-badge" aria-label={L(`软件版本 ${APP_VERSION}`, `Software version ${APP_VERSION}`)}>v{APP_VERSION}</small><button className="dialog-close-button" aria-label={L("关闭设置", "Close settings")} onClick={onClose}><svg viewBox="0 0 20 20" aria-hidden="true"><path d="m5 5 10 10M15 5 5 15"/></svg></button></header>
      <div className="settings-dialog__body settings-layout">
        ...
        <section className="settings-section settings-runtime-section">{sectionHeading(L("执行引擎", "Execution engine"), L("选择工作流默认运行环境", "Choose the default workflow runtime"))}<label className="settings-primary-control"><span>{L("工作流运行时", "Workflow runtime")}</span><ThemedSelect ariaLabel={L("工作流运行时", "Workflow runtime")} value={runtimePreference} options={[{ value: "auto", label: L("自动选择（推荐）", "Auto (recommended)") }, { value: "python", label: L("Python · 完整兼容", "Python · full compatibility") }, { value: "javascript", label: L("JavaScript · 实验", "JavaScript · experimental") }]} onChange={(value) => onRuntimePreferenceChange(value as RuntimePreference)} /></label><p>{runtimePreference === "auto" ? L("本机兼容工作流优先使用 JavaScript；遇到 Python 专属节点自动回退。", "Compatible local workflows prefer JavaScript and fall back for Python-only nodes.") : runtimePreference === "javascript" ? L("纯前端执行并支持交互式图表；不兼容节点会明确提示。", "Runs fully in the frontend with interactive charts; incompatible nodes are reported explicitly.") : L("始终使用 Python，兼容 Notebook、自定义函数与完整节点目录。", "Always use Python for Notebook, custom functions, and the full node catalog.")}</p></section>

        <section className="settings-section settings-section--canvas">{sectionHeading(L("画布", "Canvas"), L("尺寸、线条与面板布局", "Size, lines and panel layout"))}<div className="settings-range-grid">{range(L("节点尺寸", "Node size"), `${Math.round(canvas.nodeScale * 100)}%`, canvas.nodeScale, 0.75, 1.4, 0.05, "nodeScale")}{range(L("端点大小", "Handle size"), `${Math.round(canvas.endpointScale * 100)}%`, canvas.endpointScale, 0.7, 1.8, 0.1, "endpointScale")}{range(L("连线粗细", "Edge width"), `${canvas.edgeWidth.toFixed(1)} px`, canvas.edgeWidth, 1, 5, 0.5, "edgeWidth")}{range(L("左侧节点栏", "Left palette"), `${Math.round(canvas.paletteWidth)} px`, canvas.paletteWidth, 176, 360, 4, "paletteWidth")}{range(L("右侧参数栏", "Right inspector"), `${Math.round(canvas.inspectorWidth)} px`, canvas.inspectorWidth, 250, 560, 4, "inspectorWidth")}{range(L("横屏参数栏", "Landscape inspector"), `${Math.round(canvas.inspectorHeight)} px`, canvas.inspectorHeight, 140, 440, 4, "inspectorHeight")}{range(L("结果区高度", "Result height"), `${Math.round(canvas.resultHeight)} px`, canvas.resultHeight, 180, 520, 4, "resultHeight")}</div><div className="settings-canvas-options"><label><span>{L("缩略图", "Mini map")}</span><ThemedSelect ariaLabel={L("缩略图", "Mini map")} value={canvas.miniMapMode} options={[{ value: "hide", label: L("默认隐藏", "Hidden") }, { value: "auto", label: L("自动显示", "Auto") }, { value: "show", label: L("始终显示", "Always") }]} onChange={(value) => onCanvasChange({ miniMapMode: value as CanvasSettings["miniMapMode"] })} /></label><label className="settings-check"><input type="checkbox" checked={canvas.showNodeInsights} onChange={(event) => onCanvasChange({ showNodeInsights: event.target.checked })} />{L("显示节点运行结果", "Show node results")}</label></div></section>
