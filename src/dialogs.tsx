    ...
    <section className="settings-dialog agent-dialog">
      ...
      <div className="settings-dialog__body">
        <section className="agent-connection"><h3>{L("模型与连接", "Model & connection")}</h3>
          ...
          <div className="agent-connection-footer"><span className="agent-connection-footer__label" aria-hidden="true" /><div className="agent-connection-footer__content"><button className="button secondary agent-test-button" disabled={testing} onClick={onTestConnection}>{testing ? L("测试中…", "Testing…") : L("尝试连接", "Test connection")}</button><small>{apiKeyManagedByHost ? L("密钥始终留在宿主机，由安全代理代发模型请求；局域网网页不会取得原始 API 密钥。", "The key stays on the host. A secure proxy sends model requests, and Remote Web never receives the raw API key.") : keyStorageHint === "keystore" ? "Android 端使用 Keystore 加密保存，应用更新后仍可读取；不会写入设置、工作流或用户文件夹。" : "桌面/网页密钥只驻留当前会话，不会写入设置、工作流或用户文件夹。"}</small>{connectionStatus && <small className={connectionStatus.startsWith("连接成功") ? "agent-success" : "agent-failure"}>{connectionStatus}</small>}</div></div>
        </section>
