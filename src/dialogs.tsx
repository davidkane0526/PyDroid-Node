    ...
    <section className="settings-dialog agent-dialog">
      ...
      <div className="settings-dialog__body">
        <section className="agent-connection"><h3>{L("模型与连接", "Model & connection")}</h3>
          ...
          <div className="agent-connection-footer"><button className="button secondary agent-test-button" disabled={testing} onClick={onTestConnection}>{testing ? L("测试中…", "Testing…") : L("尝试连接", "Test connection")}</button><div className="agent-connection-note"><small>{apiKeyManagedByHost ? L("密钥始终留在宿主机，由安全代理代发模型请求；局域网网页不会取得原始 API 密钥。", "The key stays on the host. A secure proxy sends model requests, and Remote Web never receives the raw API key.") : keyStorageHint === "keystore" ? "Android 端使用 Keystore 加密保存，应用更新后仍可读取；不会写入设置、工作流或用户文件夹。" : "桌面/网页密钥只驻留当前会话，不会写入设置、工作流或用户文件夹。"}</small>{connectionStatus && <small className={connectionStatus.startsWith("连接成功") ? "agent-success" : "agent-failure"}>{connectionStatus}</small>}</div></div>
        </section>
...
function McpConnectionSection({ enabled, token, info, available, language, onEnabledChange, onTokenChange, embedded = false }: {
  ...
  const localDisplay = localEndpoint ?? L("启动后显示", "Shown after startup");
  const lanDisplay = lanEndpoint ?? L("启动后显示", "Shown after startup");
  ...
  return <>
    <section className={`settings-section ${embedded ? "agent-mcp-section" : "settings-mcp-summary"}`}><div className="settings-mcp-heading-row"><div className="settings-section__heading"><h3>{embedded ? L("MCP 服务器", "MCP Server") : "MCP Server"}</h3>{!embedded && <small>{L("完整 Core", "Full Core")}</small>}</div><button type="button" className="settings-help-button" aria-label={L("MCP 连接帮助", "MCP connection help")} title={L("连接帮助", "Connection help")} onClick={() => setHelpOpen(true)}><svg viewBox="0 0 20 20" aria-hidden="true"><circle cx="10" cy="10" r="7.25"/><path d="M7.9 7.55A2.25 2.25 0 0 1 10.1 6c1.35 0 2.35.78 2.35 1.92 0 1.5-1.55 1.8-2.15 2.75-.2.3-.25.55-.25 1.05"/><circle className="settings-help-button__dot" cx="10.05" cy="14.2" r=".7"/></svg></button></div>
      <div className="settings-mcp-value settings-mcp-value--token"><small>Token</small><div><input type="text" value={token} maxLength={256} placeholder={L("输入固定 Token", "Enter a fixed Token")} onChange={(event) => onTokenChange(event.target.value.replace(/[\r\n]/g, ""))} /><button type="button" className="settings-copy-button" disabled={!token} aria-label={L("复制 Token", "Copy Token")} title={L("复制 Token", "Copy Token")} onClick={() => void copyValue("Token", token)}><svg viewBox="0 0 20 20" aria-hidden="true"><rect x="6.1" y="5.5" width="8.4" height="9" rx="1.4"/><path d="M5.7 12.2H4.9a1.4 1.4 0 0 1-1.4-1.4V4.9a1.4 1.4 0 0 1 1.4-1.4h5.9a1.4 1.4 0 0 1 1.4 1.4v.6"/></svg></button></div></div>
      <div className="settings-mcp-values">
        <div className="settings-mcp-value"><small>Local</small><div><strong><code title={localEndpoint ?? undefined}>{localDisplay}</code></strong><button type="button" className="settings-copy-button" disabled={!localEndpoint} aria-label={L("复制 Local 地址", "Copy Local URL")} title={L("复制 Local 地址", "Copy Local URL")} onClick={() => localEndpoint ? void copyValue("Local", localEndpoint) : undefined}><svg viewBox="0 0 20 20" aria-hidden="true"><rect x="6.1" y="5.5" width="8.4" height="9" rx="1.4"/><path d="M5.7 12.2H4.9a1.4 1.4 0 0 1-1.4-1.4V4.9a1.4 1.4 0 0 1 1.4-1.4h5.9a1.4 1.4 0 0 1 1.4 1.4v.6"/></svg></button></div></div>
        <div className="settings-mcp-value"><small>LAN</small><div><strong><code title={lanEndpoint ?? undefined}>{lanDisplay}</code></strong><button type="button" className="settings-copy-button" disabled={!lanEndpoint} aria-label={L("复制 LAN 地址", "Copy LAN URL")} title={L("复制 LAN 地址", "Copy LAN URL")} onClick={() => lanEndpoint ? void copyValue("LAN", lanEndpoint) : undefined}><svg viewBox="0 0 20 20" aria-hidden="true"><rect x="6.1" y="5.5" width="8.4" height="9" rx="1.4"/><path d="M5.7 12.2H4.9a1.4 1.4 0 0 1-1.4-1.4V4.9a1.4 1.4 0 0 1 1.4-1.4h5.9a1.4 1.4 0 0 1 1.4 1.4v.6"/></svg></button></div></div>
        <div className="settings-mcp-value settings-mcp-value--header-span"><small>Header</small><div><strong><code>X-PyDroid-Token</code></strong></div></div>
      </div>
      <div className="settings-mcp-enable-row"><button type="button" className={`button secondary settings-mcp-toggle-button ${enabled && available ? "is-active" : ""}`} disabled={!available || !token.trim()} onClick={() => onEnabledChange(!(enabled && available))}>MCP Server</button><span className={`settings-mcp-status ${enabled && available ? "is-active" : ""}`}>{enabled && available ? L("已开启", "Enabled") : L("未开启", "Disabled")}</span></div>
    </section>
