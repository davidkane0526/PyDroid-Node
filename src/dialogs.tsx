...
export function SmbDialog({ open, servers, connection, guest, rememberPassword, passwordVisible, loading, error, path, entries, selected, scannedShares, onClose, onDiscover, onSelectServer, onConnectionChange, onGuestChange, onRememberPasswordChange, onPasswordVisibleChange, onScanShares, onSelectShare, onBrowse, onImportSelection, onToggleSelected }: {
  ...
  return <div className="settings-backdrop smb-backdrop" role="dialog" aria-modal="true" aria-label="内置 SMB 文件浏览器"><section className="smb-dialog">
    ...
    <section className="smb-login"><div className="smb-section-title"><div><strong>2 · 登录并读取共享</strong><small>填写服务器与账号密码后点“读取共享列表”，像 Windows 资源管理器一样浏览所有共享。</small></div></div><div className="smb-connection"><label>服务器<input placeholder="IP 或主机名，例如 192.168.1.10 或 Noe-Lab" value={connection.server} onChange={(event) => onConnectionChange({ server: event.target.value })} /></label><label className="smb-guest"><input type="checkbox" checked={guest} onChange={(event) => onGuestChange(event.target.checked)} />访客登录</label>{!guest && <><label>域（可选）<input value={connection.domain} onChange={(event) => onConnectionChange({ domain: event.target.value })} /></label><label>用户名<input autoComplete="username" value={connection.username} onChange={(event) => onConnectionChange({ username: event.target.value })} /></label><label>密码<span className="password-field"><input type={passwordVisible ? "text" : "password"} autoComplete="current-password" value={connection.password} onChange={(event) => onConnectionChange({ password: event.target.value })} /><button type="button" aria-label={passwordVisible ? "隐藏密码" : "显示密码"} title={passwordVisible ? "隐藏密码" : "显示密码"} onClick={onPasswordVisibleChange}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.6"/>{passwordVisible && <path d="m4 4 16 16"/>}</svg></button></span></label></>}<div className="smb-login-actions"><button disabled={loading || !connection.server.trim()} onClick={onScanShares}>{loading ? "读取中…" : "读取共享列表"}</button></div></div>{scannedShares.length > 0 && <div className="smb-shares-list"><div className="smb-section-title"><div><strong>共享列表</strong><small>点击共享直接进入</small></div></div>{scannedShares.map((share) => <button key={share} className={connection.share === share ? "active" : ""} onClick={() => onSelectShare(share)}><span>▣</span><strong>{share}</strong></button>)}</div>}<div className="smb-connection"><label>共享名<input list="smb-shares" placeholder="或手动输入共享名（\\主机名\共享名 的第一段）" value={connection.share} onChange={(event) => onConnectionChange({ share: event.target.value })} /><datalist id="smb-shares">{scannedShares.map((share) => <option key={share} value={share} />)}</datalist></label><div className="smb-login-actions"><button className="primary" disabled={loading || !connection.server.trim() || !connection.share.trim()} onClick={() => onBrowse("")}>{loading ? "连接中…" : "进入所选共享"}</button></div></div></section>
