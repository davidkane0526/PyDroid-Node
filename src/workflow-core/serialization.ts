...
export function serializeWorkflowSnapshot(name: string, snapshot: WorkflowSnapshot): WorkflowDocument {
  ...
  return serializeWorkflow(name, persistent.nodes, persistent.edges, persistent.requirements ?? []);
}
...
export function parseWorkflowSnapshot(text: string, fallbackRequirements: string[] = []): WorkflowSnapshot {
  ...
  return { nodes: document.nodes, edges: document.edges, requirements: document.requirements ?? fallbackRequirements };
}
