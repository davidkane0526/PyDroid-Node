import { createContext, useCallback, useContext, useEffect, useMemo, useRef, useState, useSyncExternalStore, type ChangeEvent, type CSSProperties, type DragEvent as ReactDragEvent, type MouseEvent as ReactMouseEvent, type PointerEvent as ReactPointerEvent } from "react";
import {
  Background,
  BackgroundVariant,
  ConnectionLineType,
...
import { NodePluginManager } from "./plugins/PluginManager";
...
import { NotebookEditor } from "./ui/NotebookEditor";
import { AppErrorBoundary } from "./ui/AppErrorBoundary";
import { EdgeActionsContext, NodeAppearanceContext, NodeConnectionsContext, NodeInsightContext, NodeLayoutContext, NodeParameterContext, NodeRunContext, NodeSelectionContext, TypedGradientEdge, WorkflowNodeCard } from "./nodes/WorkflowNodeView";
import { VALUE_TYPE_COLORS } from "./nodes/valueTypeColors";
...
function FlowEditor({ session, lifecycle, resourceLibrary, tabName = "工作流 1", onAddTab, themeMode, uiThemeId, resolvedTheme, onThemeModeChange, onUiThemeChange, mcpEnabled, mcpToken, mcpInfo, mcpAvailable, onMcpEnabledChange, onMcpTokenChange }: { session: EditorWorkspaceSession; lifecycle: EditorWorkspaceLifecycleService; resourceLibrary: EditorResourceLibraryService; tabName?: string; onAddTab: () => void; themeMode: ThemeMode; uiThemeId: string; resolvedTheme: "dark" | "light"; onThemeModeChange: (mode: ThemeMode) => void; onUiThemeChange: (id: string) => void; mcpEnabled: boolean; mcpToken: string; mcpInfo: McpServerInfo | null; mcpAvailable: boolean; onMcpEnabledChange: (enabled: boolean) => void; onMcpTokenChange: (token: string) => void }) {
  ...
  const [canvasTheme, setCanvasTheme] = useState<CanvasThemeId>(() => loadAppSettings().canvasTheme);
  ...
  const addNodeFromCatalog = (nodeType: string, position?: { x: number; y: number }) => {
    ...
    node.data.label = nodeDisplayName(nodeType, node.data.label, language);
          ...
          <div className="palette-content">
            ...
            {[...catalogGroups.entries()].map(([category, specs]) => (
              <section className="palette-group" key={category}>
                <h3>{category}</h3>
                {specs.map((spec) => { const displayLabel = nodeDisplayName(spec.nodeType, spec.label, language); const resource = resourceRef(describeCatalogNode(spec.nodeType, displayLabel)); return <button draggable={false} key={spec.nodeType} title={`按住后拖到画布添加 · ${spec.pythonCallable ?? spec.nodeType}${spec.description ? ` · ${spec.description}` : ""}`} onContextMenu={(event) => onPaletteResourceContextMenu(event, resource)} onPointerDown={(event) => onPalettePointerDown(event, resource)} onPointerMove={onPalettePointerMove} onPointerUp={onPalettePointerUp} onPointerCancel={() => { clearPaletteResourceMenuHold(); clearPaletteDrag(); }}>⠿ <strong>{displayLabel}</strong>{nodeSearch && <small>{spec.pythonCallable ?? spec.nodeType}</small>}</button>; })}
              </section>
            ))}
