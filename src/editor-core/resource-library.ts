...
import { describeFlow, describeGroup, describeSavedNode, type FlowLibraryEntry, type GroupLibraryEntry, type SavedNodeEntry } from "./resource-contract";
...
export class EditorResourceLibraryService {
  ...
  private persistGroups(groups: GroupLibraryEntry[]): void {
    const custom = groups.filter((entry) => !entry.builtIn).map(cloneGroup);
  ...
  private persistSavedNodes(savedNodes: SavedNodeEntry[]): void {
    const cloned = savedNodes.map(cloneSavedNode);
  ...
  private persistFlows(flows: FlowLibraryEntry[]): void {
    const cloned = flows.map(cloneFlow);
  ...
  private loadState(): EditorResourceLibraryState {
    const customGroups = parseArray(this.storage, RESOURCE_LIBRARY_STORAGE_KEYS.groups, isGroupEntry)
      ...
      .map((entry) => ({ ...cloneGroup(entry), nodes: repairWorkflowGroupInterfaces(entry.nodes, entry.edges) }));
    const groups = [
      ...this.builtInGroups.map((entry) => ({ ...cloneGroup(entry), builtIn: true, nodes: repairWorkflowGroupInterfaces(entry.nodes, entry.edges) })),
      ...customGroups,
    ];
    return {
      flows: parseArray(this.storage, RESOURCE_LIBRARY_STORAGE_KEYS.flows, isFlowEntry).map(cloneFlow),
      groups,
      savedNodes: parseArray(this.storage, RESOURCE_LIBRARY_STORAGE_KEYS.savedNodes, isSavedNodeEntry).map(cloneSavedNode),
      revision: 0,
    };
  }
  ...
  saveGroup(entry: GroupLibraryEntry): GroupLibraryEntry {
    ...
    const normalized = { ...cloneGroup(entry), builtIn: false, nodes: repairWorkflowGroupInterfaces(entry.nodes, entry.edges) };
  ...
  saveNode(entry: SavedNodeEntry): SavedNodeEntry {
    const normalized = cloneSavedNode(entry);
  ...
  addFlowDocument(name: string, document: string, options: { limit?: number; id?: string; savedAt?: string } = {}): FlowLibraryEntry {
    ...
    this.mirrorFile(`workflows/${entry.id}.workflow.json`, document);
  ...
  replaceFlows(flows: FlowLibraryEntry[]): EditorResourceLibraryState {
    const nextFlows = flows.map(cloneFlow);
