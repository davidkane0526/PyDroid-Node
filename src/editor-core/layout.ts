...
export function arrangeStructureChildren(nodes: WorkflowNode[], direction: EditorLayoutDirection): WorkflowNode[] {
  const structures = new Map(nodes.filter((node) => isVisualStructureNodeType(node.data.nodeType)).map((node) => [node.id, node]));
  const totals = new Map<string, number>();
  for (const node of nodes) if (node.parentId && structures.has(node.parentId)) {
    const parent = structures.get(node.parentId)!;
    const branch = isIfStructureNodeType(parent.data.nodeType) ? (node.data.branch ?? "true") : "body";
  ...
  return nodes.map((node) => {
    if (structures.has(node.id)) {
      const branchCount = isIfStructureNodeType(node.data.nodeType)
        ? Math.max(totals.get(`${node.id}:true`) ?? 0, totals.get(`${node.id}:false`) ?? 0)
        : totals.get(`${node.id}:body`) ?? 0;
      const rows = isIfStructureNodeType(node.data.nodeType) || direction === "vertical" ? branchCount : Math.ceil(branchCount / 2);
    ...
    const position = isIfStructureNodeType(parent.data.nodeType)
