...
export function WorkflowEnvironmentOverlay({ tabName, environment, parameters, requirements, workspaceVariableNames, layoutRevision, onRemoveImport, onParameterExpressionChange, onRemoveParameter, onClearWorkspaceVariables, onOpenPackageManager, ui }: Props) {
  ...
  return <>
    <button ref={buttonRef} type="button" className={`environment-float-button ${open ? "active" : ""}`} style={{ left: anchor.left, top: anchor.top }} title={ui(`当前标签“${tabName}”的运行环境`, `Runtime environment for “${tabName}”`)} aria-label={ui("打开当前标签环境", "Open environment for current tab")} aria-expanded={open} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); setOpen((value) => !value); }}><svg viewBox="0 0 14 14" aria-hidden="true"><path d="M1.5 3.5H7.6M10.4 3.5H12.5M1.5 7H4.6M7.4 7H12.5M1.5 10.5H6.6M9.4 10.5H12.5"/><circle cx="9" cy="3.5" r="1.05"/><circle cx="6" cy="7" r="1.05"/><circle cx="8" cy="10.5" r="1.05"/></svg>{ui("环境", "Env")}</button>
